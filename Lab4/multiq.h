...
Boolean isEmptyMQ(struct MultiQ* mq); // tests whether all the Queues in mq are empty
int sizeMQ(struct MultiQ* mq); // returns the total number of items in the MultiQ
int sizeMQbyPriority(struct MultiQ* mq, int p); // returns the number of items in mq with the priority p.
struct Queue* getQueueFromMQ(struct MultiQ*, int p); // returns the Queue with priority p
